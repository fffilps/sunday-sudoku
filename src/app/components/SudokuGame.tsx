...
const isValidMove = (grid: Grid, row: number, col: number, value: PuzzleValue): boolean => {
  // Check row
  for (let x = 0; x < 9; x++) {
...
const generateSudokuPuzzle = (difficulty: 'easy' | 'medium' | 'hard'): number[][] => {
...
export default function SudokuGame() {
  ...
  const handleInput = (value: PuzzleValue) => {
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      const validInputs = getValidInputs(puzzleType)
      
      if (puzzleType === 'numbers' && /^[1-9]$/.test(input)) {
        handleInput(parseInt(input))
              ...
              row.map((cell, colIndex) => (
                <div
                  ...
                  className={`w-8 h-8 flex items-center justify-center border bg-slate-200 
                    ...
                    ${incorrectCells.has(`${rowIndex}-${colIndex}`) ? 'border-2 border-red-500' : ''}
                  `}
